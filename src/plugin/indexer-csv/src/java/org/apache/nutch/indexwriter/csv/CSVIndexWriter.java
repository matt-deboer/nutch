...
/**
 * Write Nutch documents to a CSV file (comma separated values).
 * 
...
public class CSVIndexWriter implements IndexWriter {
  ...
  public void open(JobConf job, String name) throws IOException {
    ...
    tmpPath = job.getLocalPath("csvindexwriter/_"
  ...
  public String describe() {
    return getClass().getSimpleName()
        ...
        + String.format("\n  %-24s : %s", CSV_FIELDS, "ordered list of fields (columns) in the CSV file")
        + String.format("\n  %-24s : %s", CSV_FIELD_SEPARATOR, "separator between fields (columns), default: , (U+002C, comma)")
        + String.format("\n  %-24s : %s", CSV_QUOTECHARACTER, "quote character used to quote fields containing separators or quotes, default: \" (U+0022, quotation mark)")
        + String.format("\n  %-24s : %s", CSV_ESCAPECHARACTER, "escape character used to escape a quote character, default: \" (U+0022, quotation mark)")
        + String.format("\n  %-24s : %s", CSV_RECORDSEPARATOR, "separator between records (rows) resp. documents, default: \\r\\n (DOS-style line breaks)")
        + String.format("\n  %-24s : %s", CSV_VALUESEPARATOR, "separator between multiple values of one field, default: | (U+007C)")
        + String.format("\n  %-24s : %s", CSV_MAXFIELDLENGTH, "max. length of a field value in characters, default: 4096")
        + String.format("\n  %-24s : %s", CSV_CHARSET, "encoding of CSV file, default: UTF-8")
        + String.format("\n  %-24s : %s", CSV_WITHHEADER, "write CSV column headers, default: true")
        + String.format("\n  %-24s : %s", CSV_OUTPATH, "output path / directory, default: csvindexwriter")
        + "\n";
  }
